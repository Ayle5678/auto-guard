...
- [ ] 四层解析矩阵测试逐层验证；机器默认文件路径注入临时目录
- [ ] `set lang en` 后配置根持久化 `lang`、回执为英文；`zh` 同理；未知值报错并列出可用值
- [ ] `guard status` 显示生效语言：单根视图一行；聚合视图按宿主各显示一行（各根语言可不同）
- [ ] 审查提示词按 config.lang 携带理由语言指令（fake reviewer 捕获断言）
- [ ] core 引擎消息（状态/历史/优化/通知/来源标签等全部用户可见文案）en 金路径断言
- [ ] 存量中文断言测试不改一字全部通过（zh 兜底契约）
