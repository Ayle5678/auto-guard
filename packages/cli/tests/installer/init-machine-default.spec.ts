...
describe('installer: ZCode statusMessage follows the install language', () => {
  it('an English install writes English spinner text into the hook entries', async () => {
    ...
    const config = JSON.parse(readFileSync(join(home, '.zcode', 'cli', 'config.json'), 'utf8')) as {
      hooks: { PreToolUse: Array<{ hooks: Array<{ statusMessage: string }> }>; SessionStart: Array<{ hooks: Array<{ statusMessage: string }> }> }
    }
    expect(config.hooks.PreToolUse[0]!.hooks[0]!.statusMessage).toBe('🛡️ auto-guard reviewing…')
    expect(config.hooks.SessionStart[0]!.hooks[0]!.statusMessage).toBe('🛡️ auto-guard session init')
  })
  ...
  it('a Chinese install keeps the Chinese spinner text', async () => {
    ...
    expect(result.code).toBe(0)
    const config = JSON.parse(readFileSync(join(home, '.zcode', 'cli', 'config.json'), 'utf8')) as {
      hooks: { PreToolUse: Array<{ hooks: Array<{ statusMessage: string }> }> }
    }
    expect(config.hooks.PreToolUse[0]!.hooks[0]!.statusMessage).toBe('🛡️ auto-guard 安全审查中…')
  })
