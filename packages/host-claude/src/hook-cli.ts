#!/usr/bin/env node
/**
 * PreToolUse hook entry for Claude Code Auto Guard.
 *
...
import { normalizeHookInput, toGuardRequest } from './claude-adapter.ts'
...
/**
 * Translate the service decision the way the zcode hook entry does, minus any
 * interactive UI: Claude Code's native confirmation box replaces ask dialogs.
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Not a parseable hook payload — this should not happen from Claude Code.
  ...
  try {
    ...
    failClosedAsk(`auto-guard 初始化失败（检查 ~/.claude/auto-guard/config.json）：${errorMessage(error)}；保守起见需要人工确认`)
    return
  }
