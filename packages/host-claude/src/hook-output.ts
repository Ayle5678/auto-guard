/**
 * Translate guard outcomes into the Claude Code PreToolUse hook output
 * protocol (verified against code.claude.com/docs/en/hooks):
 *
 *   {"hookSpecificOutput":{"hookEventName":"PreToolUse",
 *    "permissionDecision":"allow|deny|ask","permissionDecisionReason":"…"}}
 *
 * Mapping policy (identical to the zcode adapter — zcode speaks a dialect of
 * this protocol):
 *  - allow  → emit nothing and exit 0 (fast path; silence lets the normal
 *    permission flow apply)
 *  - ask    → permissionDecision "ask"; Claude Code renders its native prompt
