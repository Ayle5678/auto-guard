 ...
 * and the SessionStart entry (claude mirror of the zcode bootstrap).
 *
 * Host coupling allowed here only (ADR-0002): the claude config root, the
...
/**
 * Session/workspace identity injected by Claude Code. The payload's
 * session_id is the primary identity (used by every request); these env
 * fallbacks only steer the disk session-state directory.
 */
...
export function bootstrap(): GuardRuntime {
  ...
  // ADR-0005: the claude hook ships without native dependencies; the Light
