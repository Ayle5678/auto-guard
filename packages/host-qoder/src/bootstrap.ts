 ...
 * and the SessionStart entry (qoder mirror of the claude bootstrap).
 *
 * Host coupling allowed here only (ADR-0002): the qoder config root, the
 * disk session-state implementation (ADR-0004), the Light audit store
 * (ADR-0005, zero native dependencies for the hook) and the API-key
...
/**
 * Session/workspace identity injected by Qoder. The payload's session_id is
 * the primary identity (used by every request); these env fallbacks only
 * steer the disk session-state directory (QODER_SESSION_ID is documented in
 * docs.qoder.com/extensions/hooks; QODER_PROJECT_DIR comes from the CN CLI
 * dialect and costs nothing to honor).
 */
export function sessionIdFromEnv(): string | undefined {
  return process.env.QODER_SESSION_ID
}

export function workspaceFromEnv(): string | undefined {
  return process.env.QODER_PROJECT_DIR ?? process.env.QODER_CWD ?? process.cwd()
}
...
export function bootstrap(): GuardRuntime {
  ...
  // ADR-0005: the qoder hook ships without native dependencies; the Light
