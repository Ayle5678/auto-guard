#!/usr/bin/env node
/**
 * PreToolUse hook entry for Qoder Auto Guard.
 *
 * Invoked by Qoder once per guarded tool call (matcher:
 * Bash|Read|Write|Edit|apply_patch|run_in_terminal|read_file|create_file|search_replace).
 * Protocol (docs.qoder.com/extensions/hooks — the Claude-compatible dialect,
 * verified against the hook scripts Qoder itself ships):
 ...
 * surface as an `ask` so the native confirmation box decides, and
 * `guard off` (enabled:false in config.json) still bypasses everything even
 * when the guard itself is sick.
 */
...
import { normalizeHookInput, toGuardRequest } from './qoder-adapter.ts'
...
/**
 * Translate the service decision the way the claude hook entry does, minus any
 * interactive UI: Qoder's native confirmation box replaces ask dialogs.
 */
async function evaluate(runtime: GuardRuntime, rawCommandRequest: GuardRequest): Promise<FinalOutcome> {
  ...
  // all get final say by the human — surfaced as the native confirmation box.
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Not a parseable hook payload — this should not happen from Qoder.
  ...
  try {
    ...
    failClosedAsk(`auto-guard 初始化失败（检查 ~/.qoder/auto-guard/config.json）：${errorMessage(error)}；保守起见需要人工确认`)
    return
  }
...
function historySubject(input: ReturnType<typeof normalizeHookInput>): string | undefined {
  ...
  const subject = name === 'bash' || name === 'pwsh' || name === 'run_in_terminal'
    ? firstString('command')
    : firstString('file_path', 'filePath', 'path', 'filepath')
